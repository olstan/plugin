...
public class SpecialAttackUserPlugin extends Plugin
{
	...
	public void onGameTick(GameTick event)
	{
		...
		List<String> itemList = Arrays.asList(config.weaponIDs().split(","));
		try
		{
			if (equipmentContainer != null && itemList != null)
			{
				Item[] items = equipmentContainer.getItems();
				for (Item item : items)
				{
					String itemCheck = String.valueOf(item.getId());
					if (itemList.contains(itemCheck))
					{
